...
const bank = {
  ...
  parse(html, envelope) {
    ...
    var text2 = html.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
    var sm = text2.match(/合计\s+(-?[\d,]+\.?\d*)\/RMB\s+(-?[\d,]+\.?\d*)\/RMB\s+(-?[\d,]+\.?\d*)\/RMB\s+(-?[\d,]+\.?\d*)\/RMB/);
    if (sm) {
      billInfo.summary = {
        prevBalance: parseFloat(sm[1].replace(/,/g, "")),
        totalRepay: parseFloat(sm[2].replace(/,/g, "")),
        totalSpend: parseFloat(sm[3].replace(/,/g, "")),
        statementBalance: parseFloat(sm[4].replace(/,/g, "")),
      };
      billInfo.rawRowCount = transactions.length;
    }
    ...
    while ((sm = sectionRe.exec(html)) !== null) {
      ...
      while ((rm = rowRe.exec(sectionHtml)) !== null) {
        ...
        trans.push({
          ...
          trans_type: amount < 0 ? (desc.includes("转账") || desc.includes("还款") || desc.includes("入账") ? "REPAY" : desc.includes("退款") || desc.includes("退货") ? "REFUND" : "REPAY") : "SPEND",
        });
      }
    }
  ...
  _extractBillInfo(html, envelope, transactions) {
    ...
    if (transactions && transactions.length > 0) {
      ...
      if (allDates.length > 0) { cycleStart = allDates[0]; cycleEnd = allDates[allDates.length - 1]; }
        ...
        var cardholder = bank.defaultCardholder;
    var nm = html.match(/尊敬的([一-龥]{2,4})(?:先生|女士)/);
    if (nm) cardholder = nm[1];
    return { billDate, dueDate, billCycle, cycleStart, cycleEnd, cardLast4: "", cardholder: cardholder };
  },
};
